...
<style>
    ...
    /* [3] 월 섹션 구조 (왼쪽 월 / 오른쪽 카드) */
    .month-section {
        display: grid;
        grid-template-columns: 100px 1fr; /* 월 레이블 100px 고정 */
        gap: 30px;
        align-items: start;
    }

    /* 왼쪽 월 레이블 스타일 (스크롤 시 상단 고정 효과 가능) */
    .month-sidebar {
        position: sticky;
        top: 140px; /* 상단 헤더 아래에 딱 붙음 */
    ...
    .project-list {
        display: grid;
        grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); /* 카드 자동 배치 */
        gap: 16px;
    }
    ...
    summary { list-style: none; padding: 18px 22px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; }
    summary::-webkit-details-marker { display: none; }
    
    .card-project-title { font-size: 1.1rem; font-weight: 800; color: #1A1A1A; }
...
if not df.empty:
    ...
    for m in range(1, 7):
        ...
        # 월 섹션 시작
        main_html += f'<div class="month-section">'
        # 왼쪽 월 레이블 (Sticky)
        main_html += f'<div class="month-sidebar">{m}월</div>'
        # 오른쪽 프로젝트 리스트
        main_html += '<div class="project-list">'
        ...
        if not month_tasks.empty:
                ...
                <details class="project-card">
                    <summary>
                        <div>
                            ...
                            <div class="badge-wrapper"><div class="badge" style="background-color: {theme['main']}15; color: {theme['main']}; border: 1.5px solid {theme['main']}30;">{combined_label}</div></div>
                        </div>
            ...
            main_html += '<div style="color: #BCB8AD; font-style: italic; font-size: 0.85rem; padding-top: 15px;">예정된 프로젝트가 없습니다.</div>'
            
        main_html += '</div></div>' # project-list, month-section 종료
        
    main_html += '</div>' # main-content 종료
