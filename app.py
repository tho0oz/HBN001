...
# 2. 디자인 CSS (가로 스크롤 및 사용자 패딩 로직 반영)
st.markdown("""<style>
    /* [1] 스트림릿 기본 UI 제거 */
    header, [data-testid="stHeader"], [data-testid="stToolbar"] { display: none !important; }
    footer { display: none !important; }

    /* [2] 사용자 제공 패딩 로직 (노트북 대응 위해 min-width 추가) */
    ...
    /* 스크롤 시 내부 레이아웃이 깨지지 않도록 최소 너비 고정 */
    .roadmap-content-inner {
        min-width: 1200px; /* 노트북에서도 정렬이 유지되는 최소 너비 */
        display: flex;
    ...
    /* 제목 영역 */
    .static-header { width: 100%; padding: 10px 0 25px 0; }
    .main-title { font-size: 2rem; font-weight: 800; color: #1A1A1A; margin: 0; letter-spacing: -1.5px; }
    .sub-title { color: #6A7683; margin: 5px 0 0 0; font-weight: 500; font-size: 0.9rem; }
    ...
    .card-manager { font-size: 0.8rem; color: #1A1A1A; opacity: 0.7; font-weight: 400; margin: 0; }
    ...
    /* 스크롤바 디자인 */
    .roadmap-scroll-wrapper::-webkit-scrollbar { height: 8px; }
    .roadmap-scroll-wrapper::-webkit-scrollbar-track { background: transparent; }
    .roadmap-scroll-wrapper::-webkit-scrollbar-thumb { background: #BCB8AD; border-radius: 10px; }
...
if not df.empty:
    # --- 가로 스크롤 감싸기 시작 ---
    st.markdown('<div class="roadmap-scroll-wrapper"><div class="roadmap-content-inner">', unsafe_allow_html=True)
    ...
    if st.session_state.selected_month is None:
        ...
        with cols[0]:
            ...
            for m in range(1, 7):
                if st.button(f"{m}월", key=f"btn_{m}"):
                    ...
                    st.rerun()
                st.write('<div style="height:80px"></div>', unsafe_allow_html=True) # 버튼 간격
            
        with cols[1]:
            # 수직 스패닝 카드 그리드
            card_grid_html = '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); grid-auto-rows: 140px; gap: 20px;">'
            for _, row in df.iterrows():
                try:
                    start, end = int(row['StartMonth']), int(row['EndMonth'])
                    span = end - start + 1
                    theme_color = COLOR_PALETTE.get(str(row['Category']).strip(), COLOR_PALETTE["Default"])
                    ...
                    <details class="project-card" style="{grid_row_style}">
                        <summary><div>
                            ...
                            <div class="badge" style="background-color: {theme_color}15; color: {theme_color}; border: 1.5px solid {theme_color}30;">{row['Category']} {row['Status']}</div>
        ...
        if not month_tasks.empty:
            html_str = '<div style="display: flex; flex-direction: column; gap: 16px;">'
        ...
        else: st.info("진행 중인 프로젝트가 없습니다.")

    st.markdown('</div></div>', unsafe_allow_html=True) # 스크롤 wrapper 종료
else:
