...
/* [공통 규격] 헤더와 본문의 좌측 라인을 완벽히 일치시키기 위한 변수 */
:root {
    --side-padding: 60px;
    --month-width: 100px;
    --content-gap: 30px;
...
.sticky-header { 
    ...
    padding: 35px var(--side-padding) 25px var(--side-padding); 
    box-sizing: border-box;
}
.main-title { font-size: 1.8rem; font-weight: 800; color: #1A1A1A; margin: 0; letter-spacing: -1.2px; }
.sub-title { color: #6A7683; margin: 6px 0 0 0; font-weight: 500; font-size: 0.85rem; }

/* 메인 콘텐츠 영역 */
.main-content { 
    margin-top: 145px; 
    padding: 0 var(--side-padding) 60px var(--side-padding); 
...
.month-section { 
    display: grid; 
    grid-template-columns: var(--month-width) 1fr; 
    gap: var(--content-gap); 
    align-items: start; 
}
...
/* [중요] 프로젝트 리스트: align-items: start로 개별 확장 해결 */
.project-list { 
    display: grid; 
    grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); 
    gap: 16px; 
    align-items: start; /* 이 속성이 다른 카드가 같이 늘어나는 것을 방지함 */
}
...
.card-manager { font-size: 0.75rem; color: #1A1A1A; opacity: 0.7; margin: 0; }
